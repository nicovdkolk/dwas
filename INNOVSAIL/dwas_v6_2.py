...
x_max = [] # store max values for later conversion
for i in df_x.columns:
    x_max.append(df_x[i].max()) # store max value of each column
    df_x[i] = df_x[i]/df_x[i].max() #normalize column values
...
# set up min/max scaler with total data set for output
...
for i in range(len(x_max)):
    x_predict[:,i] = x_predict[:,i]/x_max[i] #normalize with previous max values
...
future_trg = model.predict(x_predict) # predict output
